"""Shared runtime helpers for the notebook-first GAN pipeline."""
...
def resolve_runtime_summary(cfg: Config) -> Dict[str, Any]:
    return {
...
def train_gan(
    ...
    *,
    ...
    device = get_device()
    runtime_summary = resolve_runtime_summary(cfg)
    ...
    fid_evaluator = (
        FIDEvaluator(cfg, fid_loader, device, strict=strict_fid) if cfg.fid_enabled else None
    )
    ...
    for epoch in range(1, cfg.gan_epochs + 1):
        ...
        if cfg.fid_enabled and fid_evaluator is not None and (
            epoch % cfg.fid_every == 0 or epoch == cfg.gan_epochs
        ):
...
def generate_synthetic_pool(
    ...
    *,
    ...
    generator = Generator(z_dim=cfg.z_dim, num_classes=len(resolved_class_names)).to(device)
